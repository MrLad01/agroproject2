...
const WELLNESS_TABS = ['All', 'Nutrition', 'Wellness', 'Detox', 'Immunity', 'Energy', 'Lifestyle'] as const

// Badge colours per wellness category
...
// ── Wellness Section ─────────────────────────────────────────────
function WellnessSection({ t }: { t: Theme }) {
  ...
  // The hero card is always the first Nutrition article; when filtered away it drops to a normal card
  ...
  const WellnessBadge = ({ cat }: { cat: string }) => (
    <span
  ...
  const CardFooter = ({ readTime }: { readTime: string }) => (
    <div className="flex items-center justify-between mt-3 pt-3"
      ...
      <span className="text-[11px]" style={{ color: t.mutedVal }}>{readTime}</span>
      <button
        className="text-[11px] font-semibold flex items-center gap-1 transition-opacity hover:opacity-60"
        style={{ color: t.accentVal, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
  ...
  return (
    <section className="w-full max-w-5xl px-4 sm:px-6 mb-20">
      {/* Header */}
      <div className="mb-5">
        <p className="text-[10px] font-bold uppercase tracking-[0.28em] mb-1" style={{ color: t.accentVal }}>
          Wellness & Nutrition
        </p>
        <div className="flex items-end justify-between">
          <div>
            <h2 className="eb-garamond-semibold leading-none" style={{ fontSize: 'clamp(22px,3.5vw,34px)', color: t.headingVal }}>
              Eat Well, Live Well
            </h2>
            <div className="mt-3 w-10 h-px" style={{ backgroundColor: t.accentVal }} />
          </div>
        </div>
        <p className="cormorant-garamond-light-italic text-[14px] sm:text-[15px] mt-3 max-w-lg leading-relaxed"
          style={{ color: t.mutedVal }}>
          Our resort nutritionists share expert-backed insights on the foods that fuel your body, boost your energy, and help you feel your very best.
        </p>
      </div>
      ...
      <motion.div
        ...
        {filtered.length === 0 && (
          <div className="col-span-3 text-center py-16">
            <p className="eb-garamond-semibold text-[18px]" style={{ color: t.mutedVal }}>No articles in this category</p>
          </div>
        )}

        {/* Hero card — spans 2 cols, image left + text right */}
        {heroArticle && (
          <motion.div
            ...
            <div className="relative sm:w-[55%] h-52 sm:h-auto shrink-0 overflow-hidden"
              style={{ minHeight: 220 }}>
              <Image
                src={heroArticle.img}
              ...
              <div className="absolute inset-0"
                style={{ background: 'linear-gradient(to top, rgba(10,20,5,0.4) 0%, transparent 60%)' }} />
              <WellnessBadge cat={heroArticle.cat} />
            </div>
            {/* Text — right side */}
            <div className="flex flex-col justify-center p-5 sm:p-7 flex-1">
              <p className="eb-garamond-semibold leading-snug mb-3 group-hover:opacity-70 transition-opacity"
                style={{ fontSize: 'clamp(15px,1.6vw,19px)', color: t.headingVal, fontStyle: 'italic' }}>
                {heroArticle.title}
              </p>
              <p className="cormorant-garamond-light-italic text-[13px] leading-relaxed flex-1"
                style={{ color: t.mutedVal }}>
                {heroArticle.excerpt}
              </p>
              <CardFooter readTime={heroArticle.readTime} />
            </div>
          </motion.div>
        )}
        ...
        {restArticles.map(article => (
          <motion.div
            ...
            {/* Image */}
            <div className="relative w-full h-43.75 sm:h-46.25 overflow-hidden shrink-0">
              <Image
                src={article.img}
                alt={article.title}
                fill
              ...
              />
              <div className="absolute inset-0"
                style={{ background: 'linear-gradient(to top, rgba(10,20,5,0.42) 0%, transparent 60%)' }} />
            ...
            <div className="flex flex-col flex-1 p-4 gap-2">
              ...
              <p className="cormorant-garamond-light-italic text-[12px] leading-relaxed flex-1"
                style={{ color: t.mutedVal }}>
                {article.excerpt}
              </p>
              <CardFooter readTime={article.readTime} />
            </div>
          </motion.div>
        ))}
      </motion.div>
