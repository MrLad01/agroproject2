...
const light: Theme = {
  ...
  tabActive: 'text-[#1e5e32] border-b-[#1e5e32]', tabInactive: 'text-[#999] border-b-transparent hover:text-[#1e5e32]',
...
const dark: Theme = {
  ...
  tabActive: 'text-[#7ec850] border-b-[#7ec850]', tabInactive: 'text-[#3a5a3a] border-b-transparent hover:text-[#7ec850]',
...
type RoomImage = {
  id: string; roomTypeId: string; assetId: string; order: number
  createdAt: string; asset: MediaAsset
}
...
type HeroSlide = {
  id: string; order: number; active: boolean
  assetId: string; asset: MediaAsset
}

// ── Derived suite shape for rendering ────────────────────────────
type Suite = {
  title: string; subtitle: string; imageUrl: string; tagline: string
  description: string; size: string; beds: string; bath: string
  guests: string; slug: string
}

function roomTypeToSuites(rt: RoomType): Suite[] {
  // One panel per actual room image, falling back to heroImage if none
  const images = rt.images.length > 0
    ? rt.images.map(img => img.asset.imageUrl)
    : rt.heroImage ? [rt.heroImage.imageUrl] : []

  // If no images at all, still show one panel
  const imageList = images.length > 0 ? images : ['']

  return imageList.map((imageUrl, i) => ({
    title: rt.name,
    subtitle: `Suite ${String(i + 1).padStart(2, '0')}`,
    imageUrl,
    tagline: rt.tagline ?? '',
    description: rt.description ?? '',
    size: rt.sizeSqm ? `${rt.sizeSqm} sqm` : '—',
    beds: rt.beds,
    bath: `${rt.bathrooms} Bath`,
    guests: `${rt.maxGuests} Guests`,
    slug: rt.slug,
  }))
}
...
function ImageBreadcrumbs({
  ...
  selected: number; onSelect: (i: number) => void; t: Theme
}) {
  return (
        ...
        return (
          <button key={i} onClick={() => onSelect(i)} aria-label={s.alt}
            ...
            <span className="hidden sm:block relative overflow-hidden rounded-lg" style={{
              width: active ? 72 : 48, height: active ? 48 : 32,
              transition: 'width .4s cubic-bezier(.22,1,.36,1),height .4s cubic-bezier(.22,1,.36,1)',
              boxShadow: active ? `0 0 0 2.5px ${t.thumbRing},0 6px 20px rgba(0,0,0,0.5)` : '0 2px 8px rgba(0,0,0,0.4)',
              opacity: active ? 1 : 0.45,
            }}>
              {s.url && <Image src={s.url} alt="" fill className="object-cover" sizes="80px" />}
              {active && (
...
function HeroCarousel({ t }: { t: Theme }) {
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [htRes, slidesRes] = await Promise.all([
        fetch('/api/rooms/room-hero-text'),
        fetch('/api/rooms/room-hero-slides'),
      ])
      const [ht, sl] = await Promise.all([htRes.json(), slidesRes.json()])
      if (ht) setHeroText(ht)
      if (Array.isArray(sl)) setSlides(sl)
    } catch {
  ...
  // Sorted by order, only active slides
  const activeSlides = slides
    .filter(s => s.active)
    .sort((a, b) => a.order - b.order)

  const crumbs = activeSlides.map((s, i) => ({
    url: s.asset.imageUrl,
    alt: `Agroterra Room Slide ${i + 1}`,
  }))
    ...
    <div className="relative w-full" style={{ height: 'clamp(54vh,82vw,97vh)' }}>
      <div className="overflow-hidden w-full h-full" ref={emblaRef}>
        <div className="flex h-full">
          {activeSlides.length > 0 ? activeSlides.map((s, i) => (
            ...
            <div className="relative flex-[0_0_100%] h-full bg-black/20" />
          )}
        </div>
      </div>
      ...
      <div className="absolute bottom-16 sm:bottom-20 left-0 right-0 z-10 px-6 sm:px-12 md:px-20 lg:px-28">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          {heroText.subtext || 'Where nature, comfort, and experience meet.'}
        </motion.p>
      </div>
      ...
      <div className="absolute bottom-5 sm:bottom-7 inset-x-0 z-20 flex justify-center px-4">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <ImageBreadcrumbs slides={crumbs} selected={selectedIndex} onSelect={onDotButtonClick} t={t} />
        </motion.div>
      </div>
    </div>
  )
...
// ── Sticky tab bar ────────────────────────────────────────────────
function TabBar({
  tabs, active, onChange, t,
}: {
  tabs: { key: string; label: string }[]
  active: string; onChange: (k: string) => void; t: Theme
}) {
  return (
  ...
  return (
    ...
    >
      <div className={`flex flex-col ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'} min-h-130 sm:min-h-150 lg:min-h-170`}>
        ...
        <div className={`
          relative z-10 w-full lg:w-[48%] flex flex-col justify-center
          px-6 sm:px-10 md:px-14 lg:px-12 xl:px-16 py-10 sm:py-14 lg:py-0
          ${isEven ? 'lg:-ml-[8%]' : 'lg:-mr-[8%]'}
        `}>
          <div className="relative rounded-2xl sm:rounded-3xl p-8 sm:p-10 lg:p-12 flex flex-col gap-6"
            style={{
...
function Divider({ t }: { t: Theme }) {
  return (
    <div className="flex items-center gap-4 px-6 sm:px-10 max-w-6xl mx-auto my-4 sm:my-6">
      ...
      <div className="w-1.5 h-1.5 rounded-full rotate-45" style={{ backgroundColor: t.accentVal, opacity: 0.5 }} />
      <div className="flex-1 h-px" style={{ backgroundColor: t.ruleLine }} />
    </div>
  )
}
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 pb-16 sm:pb-24">
      ...
      <motion.div
        ...
        <div className="relative w-full lg:w-[55%] overflow-hidden" style={{ minHeight: '540px' }}>
          <Image src={bg} alt="Agroterra indoor dining" fill
          ...
          <div className="absolute bottom-4 right-6 eb-garamond-semibold select-none pointer-events-none"
            style={{ fontSize: 'clamp(80px,12vw,140px)', color: 'rgba(255,255,255,0.07)', lineHeight: 1 }}>01</div>
        </div>

        <div className="relative flex flex-col justify-center gap-6 w-full lg:w-[45%] px-8 sm:px-12 py-12 lg:py-14"
          style={{ backgroundColor: t.surfaceVal }}>
          <div className="absolute top-0 left-10 right-10 h-0.5 rounded-full"
            style={{ background: `linear-gradient(to right, transparent, ${t.accentVal}50, transparent)` }} />
          <div className="flex items-center gap-3">
            <div className="w-6 h-px" style={{ backgroundColor: t.accentVal }} />
            <span className="text-[10px] font-bold uppercase tracking-[0.24em]" style={{ color: t.accentVal }}>Experience</span>
          </div>
          <div>
            ...
            <p className="cormorant-garamond-light-italic mt-2 text-[15px] sm:text-[16px] tracking-wide"
              style={{ color: t.accentVal }}>Warmth · Flavour · Tradition</p>
          </div>
          <div className="w-full h-px" style={{ backgroundColor: t.borderVal }} />
          <p className="cormorant-garamond-light-italic leading-[1.9] text-[16px] sm:text-[17px] md:text-[18px]"
            style={{ color: t.bodyVal }}>
...
export default function RoomsPage() {
  ...
  const fetchRoomTypes = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        const saved = localStorage.getItem('rooms-tab')
  ...
  const handleTab = (slug: string) => {
    setActiveSlug(slug)
    localStorage.setItem('rooms-tab', slug)
  }

  const tabs = roomTypes.map(rt => ({
    key: rt.slug,
    label: rt.label ?? rt.name,
  }))

  const activeRoomType = roomTypes.find(rt => rt.slug === activeSlug)
  const activeSuites = activeRoomType ? roomTypeToSuites(activeRoomType) : []
  ...
  return (
    <div className={`${t.page} min-h-screen eb-garamond transition-colors duration-300 overflow-x-hidden`}>
      ...
      {!loading && tabs.length > 0 && (
        <TabBar tabs={tabs} active={activeSlug} onChange={handleTab} t={t} />
      )}
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div>
              ...
              <h2 className="eb-garamond-semibold leading-none"
                ...
                {activeRoomType.label ?? activeRoomType.name}
              </h2>
            </div>
            <p className="cormorant-garamond-light-italic text-[15px]" style={{ color: t.mutedVal }}>
              {activeSuites.length} {activeSuites.length === 1 ? 'room' : 'rooms'} available
            </p>
          </motion.div>
